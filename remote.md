...
Windows アプリケーションのラジコン操作画面を使用し、Bluetooth または Wi-Fi 通信を通じてロボットに接続します。
...
1. サンプルプログラムの `controller_bluetooth\exe` フォルダを開き、`controller.exe` をダブルクリックします。  
   ![0](../images/remote/bluetooth/0.png)

2. ラジコン操作画面が表示されます。  
   ![1](../images/remote/bluetooth/1.png)

#### Wi-Fi版
1. サンプルプログラムの `controller_wifi\exe` フォルダを開き、`controller.exe` をダブルクリックします。  
   ![0](../images/remote/wifi/0.png)

2. ラジコン操作画面が表示されます。  
   ![1](../images/remote/wifi/1.png)
...
1. 必要に応じて **Bluetoothデバイス名を変更** します。  
2. **Arduino IDEの導入 > <a href="../enviroment/#%E3%82%B5%E3%83%B3%E3%83%97%E3%83%AB%E3%83%97%E3%83%AD%E3%82%B0%E3%83%A9%E3%83%A0%E3%81%AE%E5%8B%95%E3%81%8B%E3%81%97%E6%96%B9" target="_blank" rel="noopener noreferrer">サンプルプログラムの動かし方</a>** を参照し、Bluetooth 用ファームウェアを書き込んだ後、ロボットを起動します。  
...
1. Arduino IDE で **ファイル > 開く** をクリックし、プロジェクトファイル（`remote_bluetooth.ino`）を選択します。 
   ![10](../images/remote/bluetooth/10.png)

   ![11](../images/remote/bluetooth/11.png)
...
4. 他のロボットと名前が重複しないよう、"Secaro" の部分を任意の名前に変更してください。例えば：
   ...
   ```cpp
   const char* DEVICE_NAME = "Secaro_01";
   ```


### Wi-Fi版

1. 利用する Wi-Fi 環境の `ssid` と `password` に変更します。
2. 必要に応じて **Wi-Fiデバイス名を変更** します。  
3. **Arduino IDEの導入 > <a href="../enviroment/#%E3%82%B5%E3%83%B3%E3%83%97%E3%83%AB%E3%83%97%E3%83%AD%E3%82%B0%E3%83%A9%E3%83%A0%E3%81%AE%E5%8B%95%E3%81%8B%E3%81%97%E6%96%B9" target="_blank" rel="noopener noreferrer">サンプルプログラムの動かし方</a>** を参照し、Wi-Fi 用ファームウェアを書き込んだ後、ロボットを起動します。  

#### Wi-Fi環境のssidとpasswordの変更方法

ロボットを Wi-Fi 環境で検出できるよう、Wi-Fi へ接続する必要があります。

1. Arduino IDE で **ファイル > 開く** をクリックし、プロジェクトファイル（`remote_wifi.ino`）を選択します。 
   ![40](../images/remote/wifi/4.png)

   ![50](../images/remote/wifi/5.png)

2. キーボードの `Ctrl + F` を押し、検索バーに `ssid` を入力します。  
3. 以下の定義部分が見つかります。
   ...
   ```
4. それぞれを利用する Wi-Fi 環境の ssid と password に変更してください。


#### Wi-Fiデバイス名の変更方法

出荷時のロボットは、Wi-Fi デバイス名がすべて「Secaro」に設定されています。単体で使用する場合は問題ありませんが、複数台のロボットを同時に使用する環境では、接続時に
識別できるよう、それぞれ異なる名前に変更しましょう。  
...
1. ラジコン操作画面の **Bluetoothロボットを探す** ボタンをクリックし、ロボットを検索します。  
   「スキャン中」が表示され、しばらくすると検出された Bluetooth ロボットの一覧が表示されます。  
   ![2](../images/remote/bluetooth/2.png)

2. 一覧から名前が **Secaro(xx:xx:xx:xx:xx:xx)** のロボットを選び、「Bluetoothデバイスをペアリングしてください」とのメッセージが表示されます。
   **OKボタンを押さない**で、次のステップ3の手順に従って手動でペアリングを行ってください。  
   ![3](../images/remote/bluetooth/3.png)

3. Windowsの **設定** を開き、**Bluetooth とデバイス** → **デバイスの追加** を選び、Bluetooth デバイスの追加を行います。  

   ![4](../images/remote/bluetooth/4.png)  

   ![5](../images/remote/bluetooth/5.png)  

   ![6](../images/remote/bluetooth/6.png)  
   
   ![7](../images/remote/bluetooth/7.png)  

   デバイスの状態が「**ペアリング済み**」になれば、接続完了です。  
   ![8](../images/remote/bluetooth/8.png)

4. ラジコン操作画面に戻り、**OK** ボタンをクリックします。  
   ロボット Secaro のステータスが「**接続済み**」になれば、操作可能な状態です。  
   ![9](../images/remote/bluetooth/9.png)

### Wi-Fi版

1. ラジコン操作画面の **Wi-Fiロボットを探す** ボタンをクリックし、ロボットを検索します。  
   「スキャン中」が表示され、しばらくすると検出された Wi-Fi 上のロボット一覧が表示されます。  
   ![20](../images/remote/wifi/2.png)

2. 操作するロボットをクリックして選択します。  

3. 右側のステータスが「**選択中**」に変わり、操作可能な状態です。  
   ![30](../images/remote/wifi/3.png)



## ロボットを動かす
Bluetooth 版と Wi-Fi 版のいずれも、基本的な動かし方は同じです。  
ここでは、Bluetooth 版を例にして操作方法をご紹介します。  
...
- サンプルソースは `controller_xx/src` に格納されています。
...
このプログラムは、Bluetooth または、Wi-Fi 経由でコマンドを受信し、M5Atom 経由で 2 つの連続回転サーボモータを制御するものです。実行には以下の環境が必要です。
...
```cpp
const char* DEVICE_NAME = "Secaro";   // Bluetoothデバイス名
...
#### Wi-Fi版
Atom Lite とラジコン画面は Wi-Fi で接続されるため、Wi-Fi デバイスの初期化が含まれます。
